...
async def help(ctx):
	if ctx.author.id not in buyers:
		embeds = discord.Embed(title="🚀 **AICHAN BOT** 🚀", color=0xfcb103)
		...
		embed.set_footer(text=f"© Owner : !Taka | Requests By {ctx.author.name}")
...
async def Userinfo(ctx, user:discord.Member=None):
	...
	embed.set_footer(text=f"© Owner : !Taka | Info User : {user}", icon_url=ctx.author.avatar)
...
async def vpsinfo(ctx):
	...
	embed.set_author(name="Vps Info - AICHAN BOT")
	...
	embed.set_footer(text="© Owner : !Taka | Info Bot : 🚀 AICHAN BOT 🚀", icon_url=ctx.author.avatar)
...
async def Botinfo(ctx):
	embed = discord.Embed(color=0x03ff28)
	embed.set_thumbnail(url="https://i.pinimg.com/originals/8f/f2/10/8ff210a59c7a8eadc434ec303f7a86f5.jpg")
	embed.set_author(name="Bot Info - AICHAN BOT")
	embed.add_field(name="ID :", value="1015483556501409792")
	embed.add_field(name="Name :", value="AICHAN BOT")
	embed.add_field(name="Bot Owner :", value="!Taka")
...
async def Methods(ctx):
	if ctx.author.id not in buyers:
		embedc = discord.Embed(title="🚀 **AICHAN BOT** 🚀", color=0xfcb103)
		embedc.add_field(name="**Warning**",value="You Don't Have Permission To Use This Comamnd !")
		embedc.set_footer(text=f"© Owner : !Taka | Warning {ctx.author.name}")
		await ctx.reply(embed=embedc)
	else:
		embet = discord.Embed(title="🚀 **AICHAN BOT** 🚀", color=discord.Colour.random())
		embet.add_field(name="**Methods Layer4**", value="```Update```")
		embet.add_field(name="**Methods Layer7**", value="```\nSLOW\nHYPER\nUAM\nUAM-BYPASS\nHTTP-RAW\nHTTP-RAND\nHTTP-SOCKETS\nIO-STRESSER\nCLOUDFLARE\nCF-BYPASS\nKATA-DDOS```")
		embet.set_footer(text=f"© Owner : !Taka | All Methods Show")
...
async def Commands(ctx):
	if ctx.author.id not in buyers:
		...
		embed = discord.Embed(title="🚀 **AICHAN BOT** 🚀", color=discord.Colour.random())
		...
		embed.set_footer(text="© Owner : !Taka | All Methods Command", icon_url=ctx.author.avatar)
...
async def SLOW(ctx, url, time):
	if ctx.author.id not in buyers:
		...
		embed.set_footer(text=f"© Owner : !Taka | Requests By {ctx.author.name}", icon_url=ctx.author.avatar)
...
async def HTTP_SOCKETS(ctx, url, req, time):
	if ctx.author.id not in buyers:
		...
		embed.add_field(name="**Requests/ip
